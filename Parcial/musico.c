...
*
*/
int musico_alta(Musico array[], int size, int* contadorID, Orquesta arrayOrquesta[], int sizeOrq, Instrumento arrayInstrumento[], int sizeIns)
{
    int retorno=-1;
    int posicion;
    int auxInstrumento;
...
*
*/
int musico_baja(Musico array[], int sizeArray, int size)
{
    int retorno=-1;
    int posicion;
    int id;
...
int musico_modificar(Musico array[], int sizeArray, Orquesta arrayOrquesta[],int sizeOrq)
{
    ...
    int id;
    int i;
    int AuxOrquesta;                                                                                   //cambiar si no se busca por ID
    char opcion;
    if(array!=NULL && sizeArray>0)
    {
...
*
*/
int musico_listar(Musico arrayMusico[], int sizeMusico, Instrumento arrayInstrumento[], int sizeIns)
{
    int retorno=-1;
    int i;
    int j;
    if(arrayMusico!=NULL && sizeMusico>=0)
    {
        for(i=0;i<sizeMusico;i++)
        {
            ...
            {
                if(arrayInstrumento!=NULL && sizeIns>=0)
                {
                    ...
                    for(j=0;j<sizeIns;j++)
                    {
                        if(arrayMusico[i].idInstrumento==arrayInstrumento[j].idInstrumento)
                        {
                           ...
                           printf("\n ID: %d\n NOMBRE: %s\n APELLIDO: %s\n EDAD: %d\n Nombre de instrumento: %s\nTipo de instrumeno:",
                            arrayMusico[i].idMusico,arrayMusico[i].varString,arrayMusico[i].varLongString,arrayMusico[i].edadMusico,
                            arrayInstrumento[j].varString, arrayInstrumento[j].tipoInstrumento);
                            switch(arrayInstrumento[j].tipoInstrumento)
                                                {
                                                    case 1:
                                                    printf("Cuerdas\n\n");
                                                    break;
 ...
 int bajaxOrquesta(Orquesta arrayOrquesta[], int sizeOquesta,Musico arrayMusico[], int sizeMusico)
{
    ...
    orquesta_listar(arrayOrquesta,sizeMusico);
    utn_getUnsignedInt("Ingrese ID orquesta a eliminar:","ERROR",1,sizeof(int),1,sizeOquesta,2,&auxID);


     if(arrayOrquesta!=NULL && sizeOquesta>=0)
    {
        for (i = 0; i < sizeOquesta; i++)
        {
            if(arrayOrquesta[i].idOrquesta==auxID)
            {
                for(j=0; j < sizeMusico; j++)
                {
                    if(arrayMusico[j].idOrquesta==auxID)
                    {
                        arrayMusico[j].isEmpty=1;
                        arrayMusico[j].idMusico=0;
                        arrayMusico[j].varInt=0;
                        arrayMusico[j].idOrquesta=0;
                        strcpy(arrayMusico[j].varString,"");
                        strcpy(arrayMusico[j].varLongString,"");
                    }
                }
                        arrayOrquesta[i].isEmpty=1;
                        arrayOrquesta[i].idOrquesta=0;
                        arrayOrquesta[i].varInt=0;
                        strcpy(arrayOrquesta[i].varString,"");
                        strcpy(arrayOrquesta[i].varLongString,"");
            }
        retorno = 0;
        }
